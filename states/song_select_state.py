...
class SongSelectState(BaseState):
    ...
    def startup(self, persistent):
        ...
        if self.songs:
            if menu_music_was_playing:
                # ✅ Only update visuals/UI — do not reload or restart music
                self.apply_song_visuals(song_to_select_index, instant=True)
                self.selected_index = song_to_select_index
            else:
                # Normal behavior: load preview + visuals
                self.select_song(song_to_select_index, instant=True, play_preview=True)
            ...
            if os.path.isdir(folder_path):
                ...
                if os.path.exists(beatmap_path):
                    try:
                        ...
                        song_entry = {
                            "title": beatmap_data.get("title", "N/A"),
                            "artist": beatmap_data.get("artist", "N/A"),
    ...
    def select_song(self, index, instant=False, play_preview=True):
        ...
        # Only load and play preview if explicitly asked
        if play_preview:
        ...
        if self.transition_state == "static" and self.songs:
            if event.type == pygame.KEYUP:
                if event.key == pygame.K_DOWN:
                    ...
                    # Clear menu music flag when entering gameplay
    ...
    def draw_song_list(self, surface):
        if not self.songs or not self.banner_placeholder:
            return
