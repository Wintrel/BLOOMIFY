import pygame
import os
import random
import json
from settings import *
from states.base_state import BaseState
from ui.ui_manager import UIManager


class MainMenuState(BaseState):
    def __init__(self, state_manager):
        super(MainMenuState, self).__init__(state_manager)
        self.next_state = "SONG_SELECT"

        self.ui_manager = UIManager()
        self.ui_manager.load_layout("layouts/Main_Menu_Layout.json")

        self.is_transitioning_out = False
        self.transition_anim_duration = 0.5

        # Start the random menu music only if not already active
        if not self.persist.get("menu_music_active", False):
            self.start_menu_music()

    def start_menu_music(self):
        """ Scans for songs and plays a random one. """
        songs_path = "assets/beatmaps"
        if not os.path.exists(songs_path):
            return

        all_songs = []
        for folder_name in os.listdir(songs_path):
            folder_path = os.path.join(songs_path, folder_name)
            if os.path.isdir(folder_path):
                beatmap_path = os.path.join(folder_path, "beatmap.json")
                if os.path.exists(beatmap_path):
                    all_songs.append(folder_path)

        if all_songs:
            random_song_folder = random.choice(all_songs)
            beatmap_path = os.path.join(random_song_folder, "beatmap.json")
            try:
                with open(beatmap_path, 'r', encoding='utf-8') as f:
                    beatmap_data = json.load(f)

                audio_path = os.path.join(random_song_folder, beatmap_data.get("audio_path", ""))
                if os.path.exists(audio_path):
                    pygame.mixer.music.load(audio_path)
                    pygame.mixer.music.play(-1)  # loop
                    # --- Persist info for next state ---
                    self.persist['menu_music_active'] = True
                    self.persist['menu_music_beatmap_path'] = beatmap_path
            except Exception as e:
                print(f"Error starting menu music: {e}")

    def get_event(self, event):
        super().get_event(event)
        if not self.is_transitioning_out and self.transition_state == "static":
            if event.type == pygame.KEYUP or event.type == pygame.MOUSEBUTTONUP:
                self.trigger_transition_out()

    def trigger_transition_out(self):
        self.is_transitioning_out = True
        self.go_to_next_state()

        title = self.ui_manager.get_element_by_name("game_name")
        if title:
            title.animate_position((title.absolute_pos[0], -title.size[1]), self.transition_anim_duration)

        subtitle = self.ui_manager.get_element_by_name("game_desc")
        if subtitle:
            subtitle.animate_position((subtitle.absolute_pos[0], self.screen_rect.height),
                                      self.transition_anim_duration)

        prompt = self.ui_manager.get_element_by_name("click_prompt")
        if prompt:
            prompt.animate_position((prompt.absolute_pos[0], self.screen_rect.height), self.transition_anim_duration)

    def update(self, dt):
        super().update(dt)
        self.ui_manager.update(dt)

    def draw(self, surface):
        surface.fill(BLACK)
        self.ui_manager.draw(surface)
